...
# save spe
save(spe, file = here("processed-data", "13_NMF", "spe_NMF_dACC_manual.Rdata"))
...
for (i in c(26, 23, 27, 13, 43, 40, 36, 28, 9, 33, 39)){
    ...
    pdf(file = here::here("plots", "13_NMF", "SpotPlots_dACC_manual", paste0("NMF_", i, ".pdf")),
        width = 21, height = 20)
    ...
    for (j in seq_along(brains)){
        ...
        if (length(samples) == 1){
            ...
            p1 <- make_escheR(spe_1) |> add_fill(var=factor, point_size = 9) |> add_ground(var="anno_label", stroke=0.5, point_size = 9) +
                labs(color = "layer") +
            ...
            spe_1 <- speb[, which(speb$sample_id == samples[1])]
            p1 <- make_escheR(spe_1) |> add_fill(var=factor, point_size = 4) |> add_ground(var="anno_label", stroke=0.5, point_size = 4) +
                labs(color = "layer") +
                scale_fill_gradient(low = "white", high = "black") + labs(title = paste0("Sample ", samples[1]))

            spe_2 <- speb[, which(speb$sample_id == samples[2])]
            p2 <- make_escheR(spe_2) |> add_fill(var=factor, point_size = 4) |> add_ground(var="anno_label", stroke=0.5, point_size = 4) +
                labs(color = "layer") +
            ...
            p3 <- make_escheR(spe_3) |> add_fill(var=factor, point_size = 4) |> add_ground(var="anno_label", stroke=0.5, point_size = 4) +
                labs(color = "layer") +
...
for (i in 1:75) {
    ...
    if (nmf_col %in% colnames(colData(spe.temp))) {
        ...
        p <- plotColData(spe.temp, x = "anno_label", y = nmf_col) +
            ggtitle(paste0("NMF ", i, " Layer Boxplots")) +
            facet_wrap(~ spe.temp$anno_label, scales = "free_x", nrow = 1)
    } else {
...
for (i in 1:75){
    ...
    anno_label <- colData(spe.temp)$anno_label
    dat <- data.frame(NMF_i, anno_label)
    ...
    kruskal_res <- kruskal.test(NMF_i ~ anno_label, data = dat)
