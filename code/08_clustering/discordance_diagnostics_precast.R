...
non_na_indices <- !is.na(colData(spe)[precast_name])

clustering_no_na <- colData(spe)[,precast_name][non_na_indices]
...
write.table(results, file = here::here("processed-data", "08_clustering", "cluster_diagnostics", "fasthplus", "fasthplus_results_precast.csv"),
            append = TRUE,
            row.names = FALSE,
            col.names =!file.exists(here("processed-data", "08_clustering", "cluster_diagnostics", "fasthplus", "fasthplus_results_bayesspace_precast.csv")))
