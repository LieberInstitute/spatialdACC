...
# drop any gene_names in spe_DLPFC_12 not in sce
i <- intersect(rowData(spe_DLPFC_12)$gene_name,rownames(loadings))
loadings <- loadings[rownames(loadings) %in% i,]
spe_DLPFC_12 <- spe_DLPFC_12[rowData(spe_DLPFC_12)$gene_name %in% i,]

dup_genes <- rowData(spe_DLPFC_12)$gene_name[duplicated(rowData(spe_DLPFC_12)$gene_name)]
spe_DLPFC_12 <- spe_DLPFC_12[!duplicated(rowData(spe_DLPFC_12)$gene_name), ]

# match rownames in loadings
loadings <- loadings[match(rowData(spe_DLPFC_12)$gene_name,rownames(loadings)),]
...
# save spe_DLPFC_12
save(spe_DLPFC_12, file = here("processed-data", "18_PsychENCODE_NMF", "spe_DLPFC_12_NMF.Rdata"))
...
for (i in 1:50){
    ...
    pdf(file = here::here("plots", "18_PsychENCODE_NMF", "SpotPlots_DLPFC_12", paste0("NMF_", i, ".pdf")),
        width = 21, height = 20)
...
for (i in seq(1, length(plot_list), by = 5)) {
    ...
    pdf(file = here::here("plots", "18_PsychENCODE_NMF", paste0("NMF_boxplots_DLPFC_12_", i, "-", i+4, ".pdf")),
        width = 21, height = 20)
...
write.csv(kruskal_results, here::here("processed-data", "18_PsychENCODE_NMF", "kruskal_NMF_results_DLPFC_12.csv"))
