...
# remove Excit_Ambig and Excit_L3_4_5, which are taking up a lot of the weights
sce$cellType_layer <- unfactor(sce$cellType_layer)
idx <- sce$cellType_layer %in% c("Excit_ambig","Excit_L3/4/5")
sce <- sce[,!idx]
sce$cellType_layer <- as.factor(sce$cellType_layer)
...
# save
saveRDS(res, here(processed_dir, "rctd_results_dlPFC_dlPFC_noExcitAmbig.rds"))
