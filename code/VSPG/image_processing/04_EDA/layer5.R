...
pdf(here("plots/VSPG/image_processing/layer5_summary_plots.pdf"), width = 10, height = 8)
 ggplot() +
...
ggplot() +
  ...
  cell_counts <- sample_df_filtered %>%
	...
	readr::write_csv(counts, "/users/asingh/R/layer5.csv")
...
dev.off()	  
	  # Step 1: Compute total and normalize
	  total_counts <- counts %>%
	    mutate(
	      total = astrocyte + microglia + neuron + oligo + other,
	      total_per_mm2 = total / area_mm2,
	      region = recode(slide,
	                      "V10B01-087" = "DLPFC",
	                      "V12N28-333" = "dACC")
	    )

	  # Step 2: Plot normalized total counts
	  ggplot(total_counts, aes(x = region, y = total_per_mm2, fill = region)) +
	    geom_boxplot(alpha = 0.7, outlier.shape = NA) +
	    geom_jitter(width = 0.2, size = 1.5, alpha = 0.8) +
	    theme_minimal() +
	    labs(x = "Region", y = "Total cells per mm²", title = "Normalized total cell counts by region") +
	    theme(
	      axis.text.x = element_text(angle = 45, hjust = 1),
	      legend.position = "none"
	    )