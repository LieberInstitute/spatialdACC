...
dat <- read.table(here::here("processed-data", "17-2_LDSC_enrichment", "visium_aggregated_de.tsv"),header=T)

dat.norm <- t(apply(dat, 1, function(x){x/sum(x)}))
res <- apply(dat.norm, 2, is_top_10_percent)
rownames(res) <- rownames(dat.norm)
write.csv(res,here::here("processed-data", "17-2_LDSC_enrichment", "visium_score.csv"))
